...
import com.restaurantsapp.webservice.WSAddRestaurant;
 ...
 */
public class AddRestaurantFragment extends MainFragment implements View.OnClickListener {
...
//                final String numoftables = editTextNumberOfTable.getText().toString().trim();
                ...
                } else if (arrayListFileBanner.isEmpty() || arrayListFileMenu.isEmpty()) {
                    Toast.makeText(getActivity(), "Please select ablest one image ", Toast.LENGTH_SHORT).show();
                } else {
                    if (Utils.isOnline(getActivity())) {
                        final AsyncAddRestaurant asyncAddRestaurant = new AsyncAddRestaurant();
                        asyncAddRestaurant.execute(name);
                    } else {
    ...
    public void initActionBar() {
        ((MainActivity) getActivity()).setTopBar(getString(R.string.add_restaurant), true);
    }
    ...
    private class AsyncAddRestaurant extends AsyncTask<String, Void, Void> {
        ...
        private WSAddRestaurant wsAddRestaurant;
        ...
        protected Void doInBackground(String... params) {
            wsAddRestaurant = new WSAddRestaurant(getActivity());
            wsAddRestaurant.executeService(params[0], arrayListFileBanner, arrayListFileMenu);
            return null;
        }
        ...
        protected void onPostExecute(Void login) {
            ...
            if (wsAddRestaurant != null) {
                isError = wsAddRestaurant.isError();
                if (isError) {
                    final Intent intent = new Intent(getActivity(), MainActivity.class);
                    startActivity(intent);
                    getActivity().finish();
                } else if (wsAddRestaurant.isLogout()) {
                    Utils.displayLogoutAlertDialog(getActivity(), wsAddRestaurant.getMessage());
                } else {
                    Utils.displayAlertDialog(getActivity(), "", wsAddRestaurant.getMessage());
                }
            }
        }
    }
